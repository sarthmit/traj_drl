...
for s in range(score_seeds):
    ...
    for seed in range(downstream_seeds):
        set_seed(seed)
        idx = np.arange(162770)
        np.random.shuffle(idx)

        train_idx = idx[:130000]
        val_idx = idx[130000:]

        X_train = torch.Tensor(train_data['Representation'][0]).unsqueeze(1)
        y_train = torch.Tensor(train_data['Labels']).view(-1, 40)
        ...
        for feat in range(40):
            if os.path.exists(f'celeba_models/{args.score_lr}/{args.mode}/{args.widen_factor}/{args.granularity}/{feat}/{args.down_lr}/{s}-{seed}.pt'):
                continue
            ...
            if not os.path.exists(f'celeba_models/{args.score_lr}/{args.mode}/{args.widen_factor}/{args.granularity}/{feat}/{args.down_lr}/'):
                os.makedirs(f'celeba_models/{args.score_lr}/{args.mode}/{args.widen_factor}/{args.granularity}/{feat}/{args.down_lr}/')

            torch.save(save_dict, f'celeba_models/{args.score_lr}/{args.mode}/{args.widen_factor}/{args.granularity}/{feat}/{args.down_lr}/{s}-{seed}.pt')
            del model
