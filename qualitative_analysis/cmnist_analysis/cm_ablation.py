...
for s in range(score_seeds):
    ...
    for seed in range(downstream_seeds):
        ...
        y_train = torch.Tensor(train_data['Labels']).view(-1, 3)
        ...
        for k, feat in enumerate(sorted(mappings.keys())):
            if os.path.exists(f'cmnist_models/{args.score_lr}/{args.mode}/{args.widen_factor}/{args.granularity}/{feat}/{args.down_lr}/{s}-{seed}.pt'):
                continue
            ...
            if not os.path.exists(
                    f'cmnist_models/{args.score_lr}/{args.mode}/{args.widen_factor}/{args.granularity}/{feat}/{args.down_lr}/'):
                os.makedirs(
                    f'cmnist_models/{args.score_lr}/{args.mode}/{args.widen_factor}/{args.granularity}/{feat}/{args.down_lr}/')

            torch.save(save_dict,
                       f'cmnist_models/{args.score_lr}/{args.mode}/{args.widen_factor}/{args.granularity}/{feat}/{args.down_lr}/{s}-{seed}.pt')
            del model
