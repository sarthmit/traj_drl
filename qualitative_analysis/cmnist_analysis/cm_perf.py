        ...
        for a in range(score_seeds):
            ...
            for b in range(downstream_seeds):
                if not os.path.exists(
                        f'cmnist_models/{args.score_lr}/{args.mode}/{wf}/{args.granularity}/{args.feat}/{down_lr}/{a}-{b}.pt'):
                    print(
                        f'cmnist_models/{args.score_lr}/{args.mode}/{wf}/{args.granularity}/{args.feat}/{down_lr}/{a}-{b}.pt')
                    print('Model Does Not Exist')
                    exit()

                val_acc += torch.load(f'cmnist_models/{args.score_lr}/{args.mode}/{wf}/{args.granularity}/{args.feat}/{down_lr}/{a}-{b}.pt', map_location=device)['Validation Accuracy'] / downstream_seeds
                test_acc += torch.load(f'cmnist_models/{args.score_lr}/{args.mode}/{wf}/{args.granularity}/{args.feat}/{down_lr}/{a}-{b}.pt', map_location=device)['Test Accuracy'] / downstream_seeds
