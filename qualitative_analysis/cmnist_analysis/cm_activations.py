        ...
        for a in range(score_seeds):
            for b in range(downstream_seeds):
                if not os.path.exists(
                        f'cmnist_models/{score_lr}/{args.mode}/{args.widen_factor}/{args.granularity}/{args.feat}/{down_lr}/{a}-{b}.pt'):
                    print(
                        f'cmnist_models/{score_lr}/{args.mode}/{args.widen_factor}/{args.granularity}/{args.feat}/{down_lr}/{a}-{b}.pt')
                    print('Model Does Not Exist')
                    exit()

                val_acc += torch.load(f'cmnist_models/{score_lr}/{args.mode}/{args.widen_factor}/{args.granularity}/{args.feat}/{down_lr}/{a}-{b}.pt', map_location=device)['Validation Accuracy'] / (score_seeds * downstream_seeds)
...
for s in range(score_seeds):
    nm = f'../../diffusion_model/trained_models/{args.score_lr}/{args.mode}/cmnist_{args.widen_factor}_{args.aug}_{s}'
    ...
    y_test = torch.Tensor(test_data['Labels']).view(-1, 3)
    ...
    for seed in range(downstream_seeds):
        save_dict = torch.load(f'cmnist_models/{args.score_lr}/{args.mode}/{args.widen_factor}/{args.granularity}/{args.feat}/{args.down_lr}/{s}-{seed}.pt', map_location=device)
...
if not os.path.exists(f'cmnist_activations/{args.mode}/{args.widen_factor}/{args.granularity}/'):
    os.makedirs(f'cmnist_activations/{args.mode}/{args.widen_factor}/{args.granularity}')

torch.save(torch.Tensor(np.array(activations)), f'cmnist_activations/{args.mode}/{args.widen_factor}/{args.granularity}/{args.feat}.pt')