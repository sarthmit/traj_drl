        ...
        for a in range(score_seeds):
            for b in range(downstream_seeds):
                if not os.path.exists(
                        f'synthetic_models/{score_lr}/{args.mode}/{args.widen_factor}/{args.granularity}/{args.feat}/{down_lr}/{a}-{b}.pt'):
                    print(
                        f'synthetic_models/{score_lr}/{args.mode}/{args.widen_factor}/{args.granularity}/{args.feat}/{down_lr}/{a}-{b}.pt')
                    print('Model Does Not Exist')
                    exit()

                val_acc += torch.load(f'synthetic_models/{score_lr}/{args.mode}/{args.widen_factor}/{args.granularity}/{args.feat}/{down_lr}/{a}-{b}.pt', map_location=device)['Validation Accuracy'] / (score_seeds * downstream_seeds)
...
for s in range(score_seeds):
    nm = f'../../score_sde_pytorch/trained_models/{args.score_lr}/{args.mode}/synthetic_{args.widen_factor}_{args.aug}_{s}'
    ...
    for seed in range(downstream_seeds):
        save_dict = torch.load(f'synthetic_models/{args.score_lr}/{args.mode}/{args.widen_factor}/{args.granularity}/{args.feat}/{args.down_lr}/{s}-{seed}.pt', map_location=device)
        state_dict = save_dict['state_dict']
        opt_test = save_dict['Test Accuracy']

        model = Model(in_dim, mappings[args.feat]['num_classes']).to(device)
        print(f'Pre-Load Test Accuracy: {opt_test}')
        model.load_state_dict(state_dict)
        _, act = eval(0, test_dataloader, args.feat)
        acts.append(act.numpy())

    activations.append(torch.Tensor(np.array(acts)).mean(dim=0).numpy())

    acts = np.array(acts)
    activations.append(torch.Tensor(acts).mean(dim=0).numpy())

if not os.path.exists(f'synthetic_activations/{args.mode}/{args.widen_factor}/{args.granularity}/'):
    os.makedirs(f'synthetic_activations/{args.mode}/{args.widen_factor}/{args.granularity}')

torch.save(torch.Tensor(np.array(activations)), f'synthetic_activations/{args.mode}/{args.widen_factor}/{args.granularity}/{args.feat}.pt')